...
public class BlockMapper {
    ...
    private static final Pattern REACTION_PATTERN = Pattern.compile("@feature[^\\n]*type\\s*=\\s*\"(\\w+)\"[^\\n]*\\s*(reaction)\\s*\\w+\\s*\\{");
    private static final Pattern ROUTINE_PATTERN = Pattern.compile("(routine)\\s+(\\w+)\\s*\\([^)]*\\)\\s*\\{");

    public ReactionsFile extractBlocks(String content) {
        // TODO tackle optional propagation
    ...
    private Optional<Header> extractHeader(String content) {
        ...
        if (reactionsNameMatcher.find()) {
            int start = reactionsNameMatcher.start(1);
            int end = reactionsNameMatcher.end(1);
            return Optional.of(new Header(content.substring(start, end), content.substring(0, contentBegin)));
        }
    ...
    private CodeBlocks extractCodeBlocks(String content) {
        ...
        while (hasReaction || hasRoutine) {
            boolean reactionFound = hasReaction && (!hasRoutine || reactionMatcher.start() < routineMatcher.start());
            ...
            if (reactionFound) {
                reactions.computeIfAbsent(matcher.group(1), k -> new ArrayList<>()).add(content.substring(start, end));
